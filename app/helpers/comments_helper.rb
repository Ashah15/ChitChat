module CommentsHelper
  ...
  def show_comments(user)
    ...
    content << content_tag(:p, content_tag(:span, user.name, class: 'f-name')
               .concat(content_tag(:i, '', class: 'fas fa-check-circle'))
               .concat(content_tag(:span, "@#{user.username}", class: 'u-name')), class: 'follower-name')
